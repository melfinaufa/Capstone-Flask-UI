...
def index():
    ...
    stats = {
        ...
        'rev_table' : df2.groupby(['Category','App'])[['Reviews','Rating']].agg({'Reviews' : 'sum','Rating' : 'mean'}).sort_values('Reviews', ascending = False).head(10).reset_index().to_html(classes=['table thead-light table-striped table-bordered table-hover table-sm'])
    }
    ...
    # bagian ini digunakan untuk menyimpan plot dalam format image.png
    plt.savefig('cat_genres.png',bbox_inches="tight") 
